...
skip
  ...
  .describe("PriceExercise Integration Tests", async function () {
    ...
    it("price feed return a positive value", async () => {
      ...
      expect(new web3.utils.BN(result._hex).toString()).to.be.a.bignumber.that.is.greaterThan(
        new web3.utils.BN(0)
      );
    });
    ...
    it("Should successfully make an external API request and get a result", async () => {
      ...
      expect(new web3.utils.BN(result._hex)).to.be.a.bignumber.that.is.greaterThan(
        new web3.utils.BN(0)
      );
    });
  });
